...
def main(slug, query, force=False):
    ...
    if os.path.exists(out_path) and not force:
        print(f"SĂRIT {slug}: fișierul există deja, nu se suprascrie automat "
              f"(adaugă --force la comandă dacă chiar vrei să-l înlocuiești).")
        return
    ...
    if not photos:
        print(f"NIMIC GASIT pentru query='{query}'")
        return
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print('Utilizare: py fetch_stock_image.py <slug> "<query engleza>" [--force]')
        sys.exit(1)
